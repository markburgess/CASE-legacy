...
if (currentsub_cell->Alive())
   ...
   {
   symbol->Draw(cell_index,SColour(current_cell->state),symbol->CANoSymbol);   
   }
}
...
{ int i, nalive, nalivesub, cell_index;
 ...
 if ((int)ITERATIONS % 10 == 0)
    ...
    visual->Printf(12, "Timesteps: %f",ITERATIONS);
    }
...
{ int favourite = cell_index, neigh_index, neigh2_index, neighsub_index, i, type=0;
  int neigh_count, current_alive, neigh1_alive, neigh2_alive, neigh3_alive,
    neigh4_alive, neigh5_alive, neighsub2_alive, neighsub5_alive, row, col;
  double P_max=0.0, P_dummy;
  double random = drand48(), P_diff_T, P_diff, highest = 0.0, beta, deltaE;
  ThermoCell *current_cell, *neigh_cell, *neigh2_cell, *currentsub_cell, *neighsub_cell;
...
// insert if Rectangular grid neighsub1_alive = CountAliveSub(cell_index,1) etc;
neighsub2_alive = CountAliveSub(cell_index,2);
neighsub5_alive = CountAliveSub(cell_index,5);
// We only need second and fifth neighbours; other neighbours are forbidden.
...
int neigh_neighsub2_alive[6];
int neigh_neighsub5_alive[6];
...
for (neigh_index=0; neigh_index < 6; neigh_index++)
   ...
   neigh_neighsub2_alive[neigh_index] = CountAliveSub(neigh_array.array[neigh_index],2);
   neigh_neighsub5_alive[neigh_index] = CountAliveSub(neigh_array.array[neigh_index],5);
   }

// Calculate the adsorption energy of the current cell, surface and subsurface:

grid->QueryGridPosition(cell_index,row,col);  
current_cell->E = AdsorptionEnergy(neigh1_alive, neigh2_alive, neigh3_alive, neigh4_alive, neigh5_alive, neighsub2_alive, neighsub5_alive, (row-col)%2 );
if (current_cell->Alive()) { current_alive = 1; } else { current_alive = 0; }
currentsub_cell->E = AdsorptionEnergySub(current_alive, neigh1_alive, neigh2_alive, neigh3_alive, neigh4_alive, neigh5_alive, neighsub2_alive, neighsub5_alive);
...
if (! current_cell->Alive())
   ...
   switch (method)
	 ...
	 for (neigh_index = 0; neigh_index < neigh_array.GetSize(); neigh_index++)
	    {
	    if (neigh_array.array[neigh_index] == environ->NoCell)
	       {
	    ...
	    neigh_cell->E = AdsorptionEnergy(neigh_neigh1_alive[neigh_index], neigh_neigh2_alive[neigh_index], neigh_neigh3_alive[neigh_index], neigh_neigh4_alive[neigh_index], neigh_neigh5_alive[neigh_index], neigh_neighsub2_alive[neigh_index], neigh_neighsub5_alive[neigh_index], (row-col)%2 );
	    ...
	    if (! neigh_cell->Alive())// && (method == 1 || method == 2 || method == 3))
	       ...
	       switch (method)
		     ...
		     deltaE = neigh_cell->E + current_cell->E + dE_neigh1 - E_gasads1 ;
		     // Add dE_neigh1, since current and neigh are nearest neighbours
	             // E_gasads1 is a measure of the gas density
	             beta = Tscale_fudge / ((current_cell->T + neigh_cell->T)* k_B );
		  ...
		  case 2:
		     ...
		     if (!currentsub_cell->Alive() && !currentsub_cell->Forbidden())
		        {
			deltaE = neigh_cell->E + currentsub_cell->E + dE_neigh1 - E_gasads1;
			// Add dE_neigh1, since currentsub and neigh are nearest
			// neighbours.  This should be specified better! (dE_neighsub1
			// or something)
			// E_gasads1 is a measure of the gas density
			beta = Tscale_fudge / ((currentsub_cell->T + neigh_cell->T)* k_B );
			P_dummy = exp(-beta*deltaE);
			//printf("method %d neigh %f curr %f deltaE %f beta %f P_dummy %f\n",method,neigh_cell->E,current_cell->E,deltaE,beta,P_dummy);
			if ( P_dummy > P_max )
			   {
	    ...
	    if (neigh_cell->Alive()) 
		  ...
		  case 4:
		     ...
		     deltaE = current_cell->E - neigh_cell->E;
		     beta = Tscale_fudge / ((current_cell->T + neigh_cell->T)* k_B );
		     P_dummy = exp(-beta*deltaE);
		     //printf("method %d neighsub %f curr %f deltaE %f beta %f P_dummy %f\n",method,neigh_cell->E,current_cell->E,deltaE,beta,P_dummy);
		     if (P_dummy > P_max)   
	 ...
	 for (neigh2_index = 0; neigh2_index < neigh2_array.GetSize(); neigh2_index++)
	    {
	    if (neigh2_array.array[neigh2_index] == environ->NoCell)
		{
		continue; // Edge effect
		}
	 ...
	 if (!currentsub_cell->Alive() && !currentsub_cell->Forbidden())
	    {
	    deltaE = current_cell->E + currentsub_cell->E + dE_current - E_gasads1;
	    // Add dE_current, since currentsub and neigh are at the same site.
	    beta = Tscale_fudge / ((current_cell->T + currentsub_cell->T)* k_B );
	    P_max = exp(-beta*deltaE);
   ...
   //printf(" Not alive; Pmax = %f  type = %d \n",P_max,type);
   ...
   if (P_max > random)
	 ...
	 case 3:// Adsorption, current and neighsub1
...
// 6. Diffusion, current to currentsub
   ...
   switch (method)
	 ...
	 for (neigh_index = 0; neigh_index < neigh_array.GetSize(); neigh_index++)
	    ...
	    // Calculate the adsorption energy of the neighbour
	    grid->QueryGridPosition(neigh_array.array[neigh_index],row,col);  
	    neigh_cell->E = AdsorptionEnergy(neigh_neigh1_alive[neigh_index]-1, neigh_neigh2_alive[neigh_index], neigh_neigh3_alive[neigh_index], neigh_neigh4_alive[neigh_index], neigh_neigh5_alive[neigh_index], neigh_neighsub2_alive[neigh_index], neigh_neighsub5_alive[neigh_index], (row-col)%2 );
	    
	    
	    if ((method == 1) && neigh_cell->Alive())
	 	{
	 	// Associative desorption, current and neigh1:
	 	deltaE = E_gasdes1 - neigh_cell->E - current_cell->E;
		// E_gasdes1 is a measure of the gas density
	 	beta = Tscale_fudge / ((current_cell->T + neigh_cell->T)* k_B );
	 	P_dummy = exp(-beta*deltaE);
	 	if ( P_dummy > P_max )  // Find `least action'
	 	   {
	    ...
	    if (!neigh_cell->Alive())
	 	...
	 	switch (method)
	 	   {
	 	   case 2:
	 	       // Associative desorption, current and neighsub1:
	 	       if (neighsub_cell->Alive())
	 		  {
	 		  deltaE = - neighsub_cell->E - current_cell->E - dE_neigh1;
	 		  // Subtract dE_neigh1, since it has been added twice in AdsEnrg
	 		  // This should be specified better! (dE_neighsub1
	 		  // or something)
	 		  beta = Tscale_fudge / ((current_cell->T + neighsub_cell->T)* k_B );
	 		  P_dummy = exp(-beta*deltaE)*P_des0;
			  printf("method %d sub %f curr %f deltaE %f beta %f P_dummy %f\n",method,neighsub_cell->E,current_cell->E,deltaE,beta,P_dummy);
	 ...
	 break;
	  ...
	  for (neigh2_index = 0; neigh2_index < neigh2_array.GetSize(); neigh2_index++)
		...
		deltaE = E_gasdes2 - neigh2_cell->E - current_cell->E - dE_neigh2;
		// E_gasdes2 is a measure of the gas density
	  ...
	  if (currentsub_cell->Alive())
	     {
	     deltaE = - current_cell->E - currentsub_cell->E - dE_current;
	     // Subtract dE_current, since it has been added twice in AdsEnrg
	     beta = Tscale_fudge / ((current_cell->T + currentsub_cell->T)* k_B );
	     P_dummy = exp(-beta*deltaE)*P_des0;
	     if (P_dummy > P_max)   
		{
   ...
   // The roulette is playing:
   //if (type > 0) {printf(" Alive; Pmax = %f  type = %d \n",P_max,type);}
   if (P_max > random)
     {
     switch (type)
       {
	 ...
	 return;
       ...
       case 2:// Desorption, current and neighsub1
	 {
	 neighsub_cell = (ThermoCell *) subcell[neigh_array.array[favourite]];
...
if (currentsub_cell->Alive())
   ...
   for (neighsub_index = 0; neighsub_index < neighsub_array.GetSize(); neighsub_index++)
      {
      if (neighsub_array.array[neighsub_index] == environ->NoCell)
         {
	 continue; // Edge effect
	 }
	 ...
	 continue;
      ...
      deltaE = neighsub_cell->E - currentsub_cell->E;
      beta = Tscale_fudge / ((currentsub_cell->T + neighsub_cell->T)* k_B );
      P_dummy = exp(-beta*deltaE);
      if (P_dummy > P_max)   
	 {
	 P_max = P_dummy;
...
for (cell_index = 0; cell_index < cells; cell_index++)
 ...
 if ((int)ITERATIONS % 10)
    {
    visual->Printf(8,"Entropy: %f ",ENTROPY);
    }
...
{ int cell_index, alive, forbidden;
  double T,E,state;
  FILE *fp;
...
{ unsigned int i,j;
 ...
 if (test_period == 1 && string_length < 3)
    {
    j=0;
    for (i = cell_index; i < cells; i+= test_period*stepsize*(2-(j%2)))
      {
      next = (Particle *) cell[i];
      if (!next->Alive())
	 {
...
double Palladium::AdsorptionEnergy(int neigh1_alive, int neigh2_alive, int neigh3_alive, int neigh4_alive, int neigh5_alive, int subalive2, int subalive5, int fcc)
...
energy += neigh1_alive*dE_neigh1 + neigh2_alive*dE_neigh2 + neigh3_alive*dE_neigh3 + neigh4_alive*dE_neigh4 + neigh5_alive*dE_neigh5 + subalive2*dE_neighsub2 + subalive5*dE_neighsub5;

// Exception: theta = 2/3.
if (neigh2_alive == 7) // Every second 2nd neighbour is alive (from QueryNeighbours)
  {
  energy -= 7*dE_neigh2 + dE_neigh2sym;
  }
...
energy = E_sub + current_alive*dE_current + neigh1_alive*dE_neigh1 + neigh2_alive*dE_neigh2 + neigh3_alive*dE_neigh3 + neigh4_alive*dE_neigh4 + neigh5_alive*dE_neigh5 + subalive2*dE_neighsub2 + subalive5*dE_neighsub5;
