...
visual->Printf(7,"Entropy:");
visual->Printf(9,"Symmetry:");
visual->Printf(12, "Timesteps      escaped");
...
{ int i, nalive, nalivesub, cell_index;
 ...
 if ((int)ITERATIONS % 10 == 0)
    ...
    visual->Printf(13, "   %10.0f        %10d",ITERATIONS,NESCAPED);
    }
...
for (cell_index = 0; cell_index < cells; cell_index++)
 ...
 if (pbiggest < 0.001)
    ...
    {
    visual->Printf(10,"P[%d] = %f",biggest,pbiggest);
    }
}
...
{ unsigned int i;
 ...
 for (i = cell_index; i < cells; (cell_index % rows) ? i+= test_period*stepsize : i++)
    {
    next = (Particle *) cell[i];
    if (!next->Alive())
       {
